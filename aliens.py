...
class Mothership(actors.Actor):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.beam_sounds = [
        	actors.AmbientSound(self, "sounds/deathbeam_sweep.wav", auto_update=False, pitch=1.5, volume=0.5),
        	actors.AmbientSound(self, "sounds/deathbeam_whine.wav", auto_update=False)]
    
    def draw(self):
        # ship
        Draw.quad(self.x - self.WIDTH / 2, self.y, self.WIDTH, self.HEIGHT, self.Z, (0, 0, 0, 1))
        ...
        for i in range(self.BEAM_COUNT):
            Draw.quad(bx, 0, self.BEAM_WIDTH, self.y, self.BEAM_Z, self.BEAM_COLORS[(self.beam_rotate + i) % len(self.BEAM_COLORS)])
            bx += self.BEAM_WIDTH
...
class Turret(actors.Actor):
	...
	def __init__(self, *args, **kwargs):
		actors.Actor.__init__(self, *args, **kwargs)
		self.fire_sound = actors.Sound(self, "sounds/turret_fire.wav")
		self.volley_rounds = 0
		self.volley_time = Game.time + self.VOLLEY_DELAY
	
	def fire(self, normal_x, normal_y, spawn_time):
		self.fire_sound.play()
		bullet = Game.spawn(particles.TurretBullet(self.x, self.y, spawn_time))

...
class Civilian(Actor):
    ...
    def on_damage(self, actor):
        if self.player or isinstance(actor, Mothership):
            Sound(self, "sounds/civilian_death.wav", volume=0.5).play()
            self.detach()
            self.COLLIDE_WITH_ACTORS = False
            self.COLOR = (0, 0, 0, 1)
            self.dead = True
...
class RescuePlatform(Actor):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.activate_sound = Sound(
            self, "sounds/rescue_platform_activate.wav")
        self.ping_sound = Sound(self, "sounds/rescue_platform_ping.wav")
        self.next_ping_sound = 0
        self.teleport_sound = Sound(
            self, "sounds/rescue_platform_teleport.wav")

...
class Particle(Actor):
    ...
    def draw(self):
        ...
        if self.LIFE_FADE:
            alpha *= life
        Draw.quad(self.x, self.y, self.WIDTH, self.HEIGHT, self.Z, (self.COLOR[0], self.COLOR[1], self.COLOR[2], alpha))
...
class TurretBullet(Particle):
    ...
    def draw_callback(self, nx, ny, z):
        glLineWidth(self.BOLT_WIDTH)
        glBegin(GL_LINES)
        glColor4f(self.COLOR[0], self.COLOR[1], self.COLOR[2], 1)
        glVertex3f(self.x, self.y, z)
        glColor4f(self.COLOR[0] * 0.7, self.COLOR[1] * 0.7, self.COLOR[2] * 0.7, 0)
        glVertex3f(self.x + nx * self.BOLT_LENGTH, self.y + ny * self.BOLT_LENGTH, z)
        glColor3f(1, 1, 1)
        glEnd()
